"""add_nodes: batched embedding, single persist, alignment."""
...
def test_add_batch_returns_ids_and_inserts():
    ...
    ids = tree.add_nodes([ContentNode(i, f"doc-{i}") for i in range(50)])
    assert len(ids) == 50
...
def test_add_batch_uses_single_backend_call():
    ...
    tree.add_nodes([ContentNode(i, f"doc-{i}") for i in range(30)])
...
def test_add_batch_persists_once():
    ...
    tree.add_nodes([ContentNode(i, f"doc-{i}") for i in range(40)])
    assert saves["n"] == 1  # single snapshot, not 40


def test_add_nodes_metadata_and_id_alignment():
    tree = EmbedTree(embedder=FakeTextEmbedder(dim=16), config=TreeConfig(leaf_capacity=100, max_branches=4))
    ids = tree.add_nodes([
        ContentNode(10, "a", {"k": "a"}),
        ContentNode(20, "b", {"k": "b"}),
        ContentNode(30, "c", {"k": "c"}),
    ])
    assert ids == [10, 20, 30]
...
def test_add_batch_accepts_string_item_ids():
    tree = EmbedTree(embedder=FakeTextEmbedder(dim=16), config=TreeConfig(leaf_capacity=100, max_branches=4))
    ids = tree.add_nodes([ContentNode("md5-a", "a"), ContentNode("md5-b", "b")])
...
def test_add_batch_falls_back_for_plain_callable():
    ...
    ids = tree.add_nodes([ContentNode("a", "a"), ContentNode("bb", "bb"), ContentNode("ccc", "ccc")])
...
def test_add_batch_crosses_pca_warmup():
    ...
    def embed(text):
        c, i = (int(part) for part in text.split(":"))
    ...
    tree.add_nodes([ContentNode(f"{c}:{i}", f"{c}:{i}") for c in range(4) for i in range(20)])  # 80 > warmup
